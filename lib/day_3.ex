defmodule Day3 do
  ...
  defp calculate_distance(1), do: 0
  defp calculate_distance(n) do
    index = n - 1
    offset = calculate_offset(index)

    triangle(index, offset) + offset
  end

  defp calculate_offset(x) do
    (:math.sqrt(x) - 1) / 2
    |> Float.ceil
    |> trunc
  end

  defp triangle(x, height) do
    abs(rem(x, height * 2) - height)
  end
end
